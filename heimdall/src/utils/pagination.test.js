...
const expected = ( params, result ) => expect(pagination(params)).to.be.deep.equal(result)
...
test('pagination({ total: 6, activePage: 5 }) should return [1, "...", 3, 4, 5, 6]', () => {
...
test('pagination({ total: 27, activePage: 9 }) should return [1, "...", 8,9, 10, "...", 27]', () => {
  const params = { total: 27, activePage: 9}
  const result = [1, '...', 8, 9, 10, '...', 27]
  expected( params, result )
})

test('pagination({ total: 27 }) should return [1, 2, 3, "...", 27]', () => {
  const params = { total: 27 }
  const result = [1, 2, 3, '...', 27]
  expected( params, result )
})
...
test('pagination({ total: 10, activePage: "1aa" }) should throw an error', () => {
  const params = { total: 10, activePage: '1aa' }
